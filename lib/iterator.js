import { isGeneratorFunction, isIterator, hasOwnProperty, propertyIsEnumerable, merge } from './util';
...
TypeAdaptorStorage.prototype = {
  ...
  getAdaptor: function(tp, funcName) {
    var tries = [new t.Type(tp.type, tp.cls, tp.sub), new t.Type(tp.type, tp.cls), new t.Type(tp.type)];
    while (tries.length) {
